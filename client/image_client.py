...
def validate_prompt(prompt: str) -> None:
    """校验绘图 API 的英文-only 提示词要求。"""

    if not prompt.strip():
        raise ImageDrawerError("提示词不能为空。")
    if _BLOCKED_PROMPT_RE.search(prompt):
        raise ImageDrawerError("绘图接口要求英文提示词，不能包含中文、日文假名、韩文或全角字符。")


def _build_chat_url(base_url: str, field_name: str = "draw_api.base_url") -> str:
...
async def generate_image(
    ...
    body = {
        "model": api.model,
        "messages": [{"role": "user", "content": json.dumps(draw_params, ensure_ascii=False)}],
        "stream": False,
...
async def create_vibe_cache(
    ...
    for entry in result.vibe_cache_ids:
        ...
        if cache_id:
            return cache_id
    raise ImageDrawerError("绘图接口没有返回 vibe_cache_ids，无法初始化 Vibe 预设。")
...
def encode_image_file(path: str) -> str:
    ...
    if not file_path.exists():
        raise ImageDrawerError(f"参考图不存在：{path}")
